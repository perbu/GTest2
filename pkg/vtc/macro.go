...
// MacroStore manages macro definitions and expansion
...
func (ms *MacroStore) Define(name, value string) {
	ms.mutex.Lock()
...
func (ms *MacroStore) Definef(name, format string, args ...interface{}) {
	ms.Define(name, fmt.Sprintf(format, args...))
}

// Get retrieves a macro value
func (ms *MacroStore) Get(name string) (string, bool) {
	ms.mutex.RLock()
...
func (ms *MacroStore) All() map[string]string {
	ms.mutex.RLock()
	defer ms.mutex.RUnlock()
...
func (ms *MacroStore) Expand(logger *logging.Logger, text string) (string, error) {
...
func (ms *MacroStore) expandDynamic(logger *logging.Logger, name string) (string, bool) {
...
func (ms *MacroStore) Clone() *MacroStore {
	ms.mutex.RLock()
	defer ms.mutex.RUnlock()
...
func (ms *MacroStore) Merge(other *MacroStore) {
	ms.mutex.Lock()
	defer ms.mutex.Unlock()
...
func (ms *MacroStore) MustExpand(logger *logging.Logger, text string) string {
...
func (ms *MacroStore) ExpandOrDefault(logger *logging.Logger, text, defaultValue string) string {
	result, err := ms.Expand(logger, text)
	if err != nil {
		return defaultValue
	}
...
func (ms *MacroStore) Exists(name string) bool {
	ms.mutex.RLock()
	defer ms.mutex.RUnlock()
	_, ok := ms.macros[name]
	return ok
...
func (ms *MacroStore) DefineMultiple(macros map[string]string) {
	ms.mutex.Lock()
	defer ms.mutex.Unlock()
